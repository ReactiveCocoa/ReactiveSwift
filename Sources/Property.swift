...
extension PropertyProtocol {
	...
	/// Forward values from `self` until `trigger` sends a `value` or
	/// `completed` event, at which point the returned property will stop
	/// updating.
	///
	/// - parameters:
	...
	///
	/// - returns: A property that will forward values until `trigger` sends
	///            `value` or `completed` events.
	public func take(during lifetime: Lifetime) -> Property<Value> {
	...
	/// updating.
	///
	/// - important: The current value of `self` is guaranteed to be forwarded
	///              as the initial value of the resulting property.
	///
	/// - parameters:
	///   - trigger: A signal whose `value` or `completed` events will stop the
	///              delivery of `value` events from `self`.
	...
	/// - returns: A property that will forward values until `trigger` sends
	///            `value` or `completed` events.
	public func take(until trigger: Signal<(), NoError>) -> Property<Value> {
		return take(until: SignalProducer(signal: trigger))
	}
	...
	/// - returns: A property that will forward values until `trigger` sends
	///            `value` or `completed` events.
	public func take(until trigger: SignalProducer<(), NoError>) -> Property<Value> {
		// A custom implementation of `take(until:)` for properties, which adds a
		// guarantee of always forwarding the first value.
		//
		// This is necessitated by the fact that:
		//
		//   1. `SignalProducer.take(until:)` uses right associative lifting; and
		//   2. The produced signal of `trigger` and that of `self` are concurrent,
		//      and the `trigger` may send trigger events when starting.
		//
		// These may cause the unsafe initializer of composed properties to fail,
		// which assumes at least one value would be sent synchronously.
		return lift { producer in
			return SignalProducer { observer, disposable in
				typealias Info = (hasSendInitial: Bool, shouldComplete: Bool)
				let state = Atomic<Info>((false, false))

				producer.startWithSignal { innerSignal, innerDisposable in
					disposable += innerDisposable

					disposable += trigger.start { event in
						switch event {
						case .value, .completed:
							let shouldSendCompleted: Bool = state.modify { state in
								state.shouldComplete = true
								return state.shouldComplete && state.hasSendInitial
							}
