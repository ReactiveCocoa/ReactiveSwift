...
public final class Action<Input, Output, Error: Swift.Error> {
	...
	private let eventsObserver: Signal<Event<Output, ActionError<Error>>, NoError>.Observer
	...
	public let events: Signal<Event<Output, ActionError<Error>>, NoError>
	...
	public let errors: Signal<ActionError<Error>, NoError>
	...
	public init<P: PropertyProtocol>(enabledIf property: P, _ execute: @escaping (Input) -> SignalProducer<Output, Error>) where P.Value == Bool {
		...
		(events, eventsObserver) = Signal<Event<Output, ActionError<Error>>, NoError>.pipe()
	...
	///         and nothing will be sent upon `values` for that particular signal.
	///
	/// - parameters:
	...
	public func apply(_ input: Input) -> SignalProducer<Output, ActionError<Error>> {
		return SignalProducer { observer, disposable in
			...
			if !startedExecuting {
				let disabledEvent: Event<Output, ActionError<Error>> = .failed(.disabled)
				observer.action(disabledEvent)
				self.eventsObserver.send(value: disabledEvent)
				return
			}
			...
			self.executeClosure(input).startWithSignal { signal, signalDisposable in
				...
				signal.observe { event in
					let errorMappedEvent = event.mapError(ActionError.producerFailed)
					observer.action(errorMappedEvent)
					self.eventsObserver.send(value: errorMappedEvent)
				}
			}
