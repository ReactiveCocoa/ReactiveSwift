...
public final class Action<Input, Output, Error: Swift.Error> {
	...
	private let execute: (Action<Input, Output, Error>, SignalProducer<Input, Error>) -> SignalProducer<Output, ActionError<Error>>
	...
	public init<State: PropertyProtocol>(state: State, enabledIf isEnabled: @escaping (State.Value) -> Bool, execute: @escaping (State.Value, Input) -> SignalProducer<Output, Error>) {
		...
		self.execute = { action, inputProducer in
			return SignalProducer { observer, lifetime in
				...
				let interruptHandle = inputProducer
					.take(first: 1)
					.flatMap(.race) { execute(state, $0) }
					.start { event in
	...
	/// Create an `Action` that wraps an inner `Action` with an additional availability
	/// condition applied.
	///
	/// - parameters:
	///   - inner: The inner `Action`.
	///   - isEnabled: An optional additional `Action` availability condition.
	public convenience init<Enabled: PropertyProtocol>(
		inner: Action<Input, Output, Error>,
		enabledIf isEnabled: Enabled
	) where Enabled.Value == Bool {
		self.init(inner: inner, enabledIf: isEnabled) { SignalProducer(value: $0) }
	}

	/// Create an `Action` that pipes its output value to an inner `Action` as its input
	/// value. The created `Action` shares its availability and execution state with the
	/// inner `Action`.
	///
	/// - important: The inner `Action` would take only the first value yielded by the
	///              producer returned by `execute`.
	///
	/// - parameters:
	///   - inner: The inner `Action`.
	///   - execute: A closure that produces a unit of work that would yield a value for
	///              the inner `Action` to start its work.
	public convenience init<InnerInput>(
		inner: Action<InnerInput, Output, Error>,
		execute: @escaping (Input) -> SignalProducer<InnerInput, Error>
	) {
		self.init(inner: inner, enabledIf: Property(value: true), execute: execute)
	}
	...
	/// - parameters:
	///   - inner: The inner `Action`.
	///   - isEnabled: An additional `Action` availability condition.
	///   - execute: A closure that produces a unit of work that would yield a value for
	///              the inner `Action` to start its work.
	public init<InnerInput, Enabled: PropertyProtocol>(
		inner: Action<InnerInput, Output, Error>,
		enabledIf isEnabled: Enabled,
		execute: @escaping (Input) -> SignalProducer<InnerInput, Error>
	) where Enabled.Value == Bool {
		deinitToken = Lifetime.Token()
		eventsObserver = Signal.Observer()

		(disabledErrors, disabledErrorsObserver) = Signal.pipe()
		inner.disabledErrors.observe(disabledErrorsObserver)

		lifetime = inner.lifetime
		(events, values, errors, completed) = (inner.events, inner.values, inner.errors, inner.completed)
		isExecuting = inner.isExecuting
		self.isEnabled = isEnabled.and(inner.isEnabled)
		...
		self.execute = { action, input in
			...
			return inner.execute(inner, input.take(first: 1).flatMap(.race, execute))
		}
	}
	...
	public func apply(_ input: Input) -> SignalProducer<Output, ActionError<Error>> {
		return execute(self, SignalProducer(value: input))
	}
}
